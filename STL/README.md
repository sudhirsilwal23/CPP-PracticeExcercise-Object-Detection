...
### 10. Efficient Lookup of Objects with `std::map`  
...
### 11. Processing Detections in Order with `std::queue`  
...
### 19. Copying Bounding Boxes to a Sub-vector (`std::copy`)  
Use `std::copy` to copy the first `N` bounding boxes from a larger vector.  

---

### 20. Counting Specific Object Labels (`std::count_if`)  
...
### 22. Finding a Specific Object ID (`std::find_if`)  
Find the bounding box with a given ID.  

---

### 23. Finding First Low-Confidence Detection (`std::find_if`)  
Locate the first detection with confidence < 0.5.  

---

### 24. Highest Confidence Score (`std::max_element`)  
Find the detection with the **highest confidence**.  

---

### 25. Smallest Bounding Box Area (`std::min_element`)  
...
### 29. Reversing Specific Bounding Boxes (`std::reverse`)  
...
### 35. Combined Exercise: Filter, Sort, and Accumulate  
